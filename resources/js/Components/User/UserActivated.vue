<template>
    <div class="flex items-center mb-4">
        <h4 class="w-full sm:w-96 font-bold text-xl">Activés</h4>
    ...
    <div v-else class="border rounded-xl overflow-hidden">
        <table calss="table-auto text-sm">
            ...
            <tbody>
                <tr v-for="(user, index) in usersActivated" :class="index % 2 === 0 ? 'bg-white' : 'bg-amber-100/50'" class="divide-x divide-[#E0DDCE]">
                    ...
                    <td class="p-4 pl-8 text-zinc-900">
                        <form @submit.prevent="deleteUser" @click="user_id = user.id">
                            <button type="submit" class="flex items-center py-0.5 rounded-xl text-sm border-2 border-zinc-900 px-4 text-zinc-900 font-semibold mr-3 hover:bg-zinc-900 hover:text-white">
