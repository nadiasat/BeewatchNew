<template>
    ...
    <button @click="modalUpdateMaterial"  class="flex items-center rounded-xl text-sm border-2 border-zinc-900 px-2 py-2 text-zinc-900 font-semibold mr-3 hover:bg-zinc-900 hover:text-white">
    ...
    <Modal v-show="modalUpdateMaterial" @close="modalUpdateMaterial = false">
        <div class="container">
            <form @submit.prevent="updateMaterial()" class="mx-8">
                ...
                <BreezeInput v-model="updateMaterialForm.address" placeholder="10 rue des Tartampions" id="address" type="text"
                class="mt-1 block w-full" required />
                ...
                <button :class="{ 'opacity-25': updateMaterialForm.processing }"
                        type="button"
                        :disabled="updateMaterialForm.processing"
...
export default {
    ...
    methods: {
        updateMaterial() {
            this.updateMaterialForm.put(route('material.update', this.material), {
                preserveState: false,
                onSuccess() {
                    console.log('success');
                    Swal.fire({
        ...
        // deleteMaterial() {
        //     Swal.fire({
        //         title: "Voulez-vous vraiment supprimer le matériel ?",
        ...
        //             this.updateMaterialForm.delete(route('material.destroy', this.material), {
        //                 preserveState: false,
        //                 onSuccess() {
        //                     console.log('success');
        //                     Swal.fire({
        ...
        //     });
        openModal() {
            this.updateMaterialForm.id = this.inventory_place.id;
            this.updateMaterialForm.name = this.inventory_place.name;
            this.updateMaterialForm.address = this.inventory_place.address;
