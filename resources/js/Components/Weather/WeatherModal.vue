<template>
    ...
    <button @click="modalShowWeather=true" class="flex items-center rounded-xl text-sm border-2 border-zinc-900 px-2 py-2 text-zinc-900 font-semibold hover:bg-zinc-900 hover:text-white">
    ...
    <Modal v-show="modalShowWeather" @close="modalShowWeather = false">
        <div class="container">
            <div class="text-center">
                <h2 class="text-2xl font-semibold mb-4">Météo du {{ currentDate }}</h2>
            </div>

            <WeatherCard v-if="weather!=null" :weather_data="weather"></WeatherCard>

            <div class="text-center">
                <button @click="modalShowWeather = false" class="text-red-1 font-semibold">Annuler</button>
            </div>
        </div>
    </Modal>
